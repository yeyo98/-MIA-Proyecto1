        ...
        if(comando[tamanio] == '~'){//ENTRA EN EL ~:~ PARA VER EL VALOR
            if(comando[tamanio+1] == ':' && comando[tamanio+2] == '~'){
                ...
                if(strcasecmp(pivote,"-size") == 0){
                    ...
                    sprintf(mensaje,"\t*** Error!! Caracteristica MKFILE \"%s\" no reconocida ****\n",pivote);
...
void Ejecutar_Comandos()
{
    ...
    if(Validar_Numero(&daLoguer.id[3]))
    {
        ...
        if(valor != -1)
        {
            ...
            if( strcasecmp(caracteristicas.cont,"no")!=0)
            {
                Ejecutar_Cont(disco,sblock,datos_particion->size,datos_particion->inicio);

                int ret = Existe_Archivo(disco,sblock,datos_particion->size,caracteristicas.path);
                if(ret != -1)
                    Mensaje_Color("=== Se creo el nuevo Archivo ===\n",CYAN);
                else
                    Mensaje_Color("*** ERROR!! Este Archivo no existe ***\n",ROJO);

                return;
            }
            if( caracteristicas.size != -1)
            {
                Ejecutar_Size(disco,sblock,datos_particion->size,datos_particion->inicio);
            }

            // SI NO PASO POR CONT O POR SIZE VERIFICO PARA SOLO CREARLO
            if(Existe_Archivo(disco,sblock,datos_particion->size,caracteristicas.path) == -1)    // PRIMERO ANALIZO SI EXISTE EL ARCHIVO
            {
                Crear_Archivo(disco,sblock,datos_particion->inicio,datos_particion->size,caracteristicas.path);
            }
            ...
            if(ret != -1)
                ...
                Mensaje_Color("*** ERROR!! Este Archivo no se creo ***\n",ROJO);
...
{
    if(Existe_Archivo(disco,sblock,size,caracteristicas.path) == -1)    // PRIMERO ANALIZO SI EXISTE EL ARCHIVO
    {
        Crear_Archivo(disco,sblock,inicio,size,caracteristicas.path);
        char texto[1000];
        ...
        {
            texto[i] = cont + 48; // PASO A ASCII DEL NUMERO
            cont++;
            if(cont == 10)
                cont = 0;
        }
        Escribir_Archivo(disco,sblock,inicio,size,caracteristicas.path,texto);
    }
}
...
void Ejecutar_Cont(FILE *disco, SuperBlock sblock, int size, int inicio)
{
    if(Existe_Archivo(disco,sblock,size,caracteristicas.path) == -1)    // PRIMERO ANALIZO SI EXISTE EL ARCHIVO
    {
        ...
        FILE* archivo;
        char auxTexto[5000];
        memset(auxTexto,'\0',sizeof(auxTexto));

        archivo = fopen(caracteristicas.cont,"r");
        ...
        while(fgets(auxTexto,5000,archivo) != NULL)
        {
            //printf("%s",auxTexto);//IMPRIMO LINEA POR LINEA DEL ARCHIVO
            strcat(texto,auxTexto);
            memset(auxTexto,'\0',sizeof(auxTexto));
        }

        fclose(archivo);

        // MANDO A ESCRIBIR EL ARCHIVO
        Crear_Archivo(disco,sblock,inicio,size,caracteristicas.path);
        Escribir_Archivo(disco,sblock,inicio,size,caracteristicas.path,texto);
    }
}
